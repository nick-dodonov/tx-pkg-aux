...
// ---------------------------------------------------------------------------
// WsSigUser — ISigUser that owns the WebSocket connection
//
...
class WsSigUser : public ISigUser
{
public:
    WsSigUser(const WsSigUser&) = default;
    WsSigUser(WsSigUser&&) = delete;
    WsSigUser& operator=(const WsSigUser&) = default;
    WsSigUser& operator=(WsSigUser&&) = delete;

    WsSigUser(PeerId id, std::shared_ptr<rtc::WebSocket> ws)
        : _id(std::move(id))
        , _ws(std::move(ws))
    {}
...
WsSigClient::WsSigClient()
    : WsSigClient(Options{})
{}

WsSigClient::WsSigClient(Options options)
    : _options(std::move(options))
{}

WsSigClient::~WsSigClient() = default;

void WsSigClient::Join(PeerId id, SigMessageHandler onMessage, SigJoinHandler onJoined)
{
    ...
    ws->onOpen([ws, id, fired, onJoinCap]() {
        ...
        *fired = true;
        ...
        auto user = std::make_shared<WsSigUser>(id, ws);
